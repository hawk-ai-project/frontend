...
export default function HistoryDetailClient({ history, detail, inspectionId = null }) {
  ...
  const source = detail.originalImageUrl || '/images/home/main-analysis.png';
  const analyzedSource = detail.annotatedImageUrl || '/images/home/main-analysis.png';
  ...
  return <div className="page-shell history-detail-page compact-detail-page">
    ...
    <div className="compact-top-grid"><article className="card compact-image-card"><div className="detail-image-grid"><DetailImage title="원본 이미지" src={source} downloadName={`inspection-${inspectionId || 'original'}-original`} onZoom={setZoomImage} /><DetailImage title="분석 이미지" src={analyzedSource} downloadName={`inspection-${inspectionId || 'analysis'}-annotated`} analyzed onZoom={setZoomImage} /></div></article><article className="card compact-summary-card"><h2>점검 요약</h2><div className="compact-meta-grid"><Meta label="처리 상태" value={<span className={`badge ${statusClass(status)}`}>{status}</span>} /><Meta label="점검 일시" value={formatDateTime(history.inspectedAt)} /><Meta label="점검 장소" value={detail.fullLocation} /><Meta label="점검자" value={detail.inspector} /><Meta label="GPS 좌표" value={detail.coordinates || '좌표 미등록'} /><Meta label="현장 위치" value={<a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(detail.fullLocation)}`} target="_blank" rel="noreferrer">지도 보기</a>} /></div><h3>탐지 결과</h3><div className="detection-tags">{detail.detections.map(([name, count]) => <span key={name}>{name} <b>{count}개</b></span>)}</div></article></div>
...
function DetailImage({ title, src, downloadName, analyzed, onZoom }) {
  return <div><h3>{title}</h3>{src ? <div className={`detail-image${analyzed ? ' analyzed' : ''}`}><Image src={src} alt={title} fill unoptimized sizes="(max-width: 720px) 100vw, 50vw" /><div><button type="button" onClick={() => onZoom(src)}>확대</button><a href={src} download={downloadName}>다운로드</a></div><span>{analyzed ? 'AI 분석 결과' : '현장 촬영 이미지'}</span></div> : <div className="detail-image-empty"><strong>{title} 없음</strong><span>{analyzed ? '저장된 AI 분석 이미지가 없습니다.' : '저장된 원본 이미지가 없습니다.'}</span></div>}</div>;
}
