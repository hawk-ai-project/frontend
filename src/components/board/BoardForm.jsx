...
export default function BoardForm({ boardId }) {
  ...
  const [inspectionPreviewLoading, setInspectionPreviewLoading] =
    useState(false);
  ...
  useEffect(() => {
      ...
      Promise.resolve().then(() => {
        try {
          ...
          if (saved?.title || saved?.content)
            setForm({ ...EMPTY_FORM, ...sanitizeBoardDraft(saved) });
        } catch {
    ...
    boardService
      ...
      .catch((error) => {
        if (!cancelled)
          setNotice(getApiErrorMessage(error, "게시글을 불러오지 못했습니다."));
      })
      .finally(() => {
  ...
  }, [authLoading, isAuthenticated, inspectionHistories, inspectionLoading]);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("hawk-ai:board-draft-ready", applyAIDraft);
  }, []);
  ...
  useEffect(() => {
    if (
      isEdit ||
      loading ||
      (!form.title && !form.content && !form.tags.length)
    )
      return;
    const timer = window.setTimeout(() => {
  ...
  const addTag = () => {
    ...
    if (tag.length > 20)
      return setErrors((current) => ({
        ...current,
        tags: "태그는 20자 이하로 입력해 주세요.",
      }));
    if (form.tags.length >= 8)
      return setErrors((current) => ({
        ...current,
        tags: "태그는 최대 8개까지 추가할 수 있습니다.",
      }));
  ...
  const generateAIDraft = async () => {
    ...
    try {
      ...
      if (selectedInspection?.imageId) {
        try {
          copiedImage = await boardService.copyInspectionImage(
      ...
      window.dispatchEvent(
        new CustomEvent("hawk-ai:board-job-started", {
  ...
  const loadInspectionHistories = async () => {
    ...
    try {
      ...
      setInspectionError(
        getApiErrorMessage(error, "점검이력을 불러오지 못했습니다."),
      );
    } finally {
  ...
  const loadInspectionPreview = async (inspection) => {
    ...
    try {
      ...
      if (requestId === inspectionPreviewRequestRef.current) {
        setInspectionPreviewError(
          "저장된 점검 이미지 파일이 없습니다. 이미지를 제외하고 글을 생성할 수 있습니다.",
        );
      }
    } finally {
      if (requestId === inspectionPreviewRequestRef.current) {
  ...
  const selectInspection = (inspectionId) => {
    ...
    setAIInput({
      ...
      priority:
        PRIORITY_LABELS[inspection.priority] || inspection.priority || "",
  ...
  if (loading || authLoading)
    return <CommonLoading message="게시글 정보를 불러오는 중..." />;
    ...
    <article className="card board-editor-card">
      <form className="wide-board-form" onSubmit={submit} noValidate>
        <div className="board-editor-fields">
          {!isEdit && (
            <label htmlFor="board-inspection-history">
              점검 이력 불러오기
              <select
                id="board-inspection-history"
                value={historyImportId}
                onChange={(event) =>
                  importInspectionHistory(event.target.value)
                }
              >
          ...
          <label className="tag-field" htmlFor="board-summary">
            요약
            <input
              id="board-summary"
              className="input"
          ...
          <label className="tag-field" htmlFor="board-tags">
            태그
            <div
              className={`tag-input-box ${errors.tags ? "input-error" : ""}`}
            >
              {form.tags.map((tag) => (
        ...
        <div className="editor-form-footer">
          <div>
            {notice && (
              <p className="board-save-notice" role="alert">
                {notice}
              </p>
            )}
          </div>
          <div className="form-actions">
            ...
            <button
              type="button"
              className="btn btn-soft"
              onClick={() => setForm(isEdit ? originalForm : EMPTY_FORM)}
            >
              초기화
            </button>
            <button
      ...
      {showAIDialog && (
        <div className="dialog-backdrop" role="presentation">
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="ai-board-title"
          >
            ...
            <div className="ai-inspection-picker">
              <div className="ai-inspection-picker-head">
                ...
                <button
                  ...
                  onClick={loadInspectionHistories}
                >
                  {inspectionLoading ? "불러오는 중..." : "새로고침"}
                </button>
              </div>
              {inspectionLoading && inspectionHistories === null && (
                <p>최근 점검이력을 불러오는 중입니다.</p>
              )}
              {inspectionError && (
                <p className="ai-inspection-error" role="alert">
                  {inspectionError}
                </p>
              )}
              {!inspectionLoading && inspectionHistories?.length === 0 && (
                <p>
                  등록된 점검이력이 없습니다. 아래 항목을 직접 입력할 수
                  있습니다.
                </p>
              )}
              {inspectionHistories?.length > 0 && (
                <label htmlFor="ai-inspection-history">
                  최근 점검이력
                  <select
                    id="ai-inspection-history"
                    value={selectedInspectionId}
                    onChange={(event) => selectInspection(event.target.value)}
                  >
                    ...
                    {inspectionHistories.map((inspection) => (
                      <option value={String(inspection.id)} key={inspection.id}>
                        ...
                        {formatInspectionDate(inspection.capturedAt)} ·{" "}
                        {inspection.status} ·{" "}
              ...
              {selectedInspectionId &&
                (() => {
                  const selected = inspectionHistories?.find(
                    (item) => String(item.id) === selectedInspectionId,
                  );
                  return selected ? (
                    <div className="ai-selected-inspection">
                      <span>
                        선택된 점검: #{selected.id} · {selected.location} ·{" "}
                        {formatInspectionDate(selected.capturedAt)}
              ...
              {inspectionPreviewLoading && (
                <p>점검 이미지를 불러오는 중입니다.</p>
              )}
              {inspectionPreviewError && (
                <p className="ai-inspection-error" role="alert">
                  {inspectionPreviewError}
                </p>
              )}
            ...
            <label htmlFor="ai-waste-summary">
              폐기물 현황
              <textarea
                id="ai-waste-summary"
                className="input"
                value={aiInput.wasteSummary}
                onChange={(event) =>
                  setAIInput((current) => ({
                    ...current,
                    wasteSummary: event.target.value,
                  }))
                }
                placeholder="예: 페트병 12개, 스티로폼 4개"
                rows={3}
              />
            </label>
            ...
            <label htmlFor="ai-notes">
              ...
              <textarea
                ...
                placeholder="추가로 반영할 사실을 입력하세요"
                rows={3}
              />
            </label>
            {aiError && (
              <p className="board-save-notice board-save-error" role="alert">
                {aiError}
              </p>
            )}
            <div className="form-actions">
