...
function validate(title, content) {
  ...
  if (!trimmedContent) errors.content = "본문을 입력해 주세요.";
    ...
    errors.content = "본문은 10000자 이하로 입력해 주세요.";
  return errors;
}

export default function BoardForm({ boardId }) {
  const isEdit = Boolean(boardId);
  const sample = prototypeBoards.find(
  ...
  const submit = (event) => {
    ...
    setNotice(
      isEdit
        ? "게시글 수정 기능을 준비하고 있습니다. 변경한 내용은 현재 서버에 저장되지 않습니다."
        : "게시판 저장 기능을 준비하고 있습니다. 작성한 내용은 현재 서버에 등록되지 않습니다.",
    );
  };
  ...
  return (
    <article className="card board-editor-card">
      {pendingDraft && (
        <section
          className="draft-restore"
          aria-labelledby="draft-restore-title"
        >
          <div>
            <strong id="draft-restore-title">
              작성 중이던 임시 저장 내용이 있습니다.
            </strong>
            <p>카테고리와 태그를 포함해 불러오시겠습니까?</p>
          </div>
        ...
        <div className="editor-form-footer">
          <div>
            <span className="draft-status">
              {isEdit ? "프론트 샘플 데이터 편집 중" : saveStatus}
            </span>
            {notice && (
              <p className="board-save-notice" role="status">
                {notice}
              </p>
            )}
          </div>
      ...
      {showDeleteConfirm && (
        <div className="dialog-backdrop" role="presentation">
          ...
          >
            <h2 id="draft-delete-title">임시 저장 내용을 삭제할까요?</h2>
            <p>작성 중인 카테고리, 제목, 태그와 본문이 모두 초기화됩니다.</p>
            <div className="form-actions">
              <button
                type="button"
                className="btn btn-secondary"
