    ...
    <footer className="site-footer">
      <div className="footer-inner">
        <section className="footer-brand" aria-labelledby="footer-brand-title">
          <Link className="footer-brand-link" href="/" aria-label="Hawk-AI HOME으로 이동">
            <span className="footer-logo"><Image src="/images/common/logo1.png" alt="Hawk-AI" width={150} height={56} /></span>
        ...
        <nav className="footer-project" aria-labelledby="footer-project-title">
          ...
          <ul>
            {projectLinks.map((link) => (
              <li key={link.href}>
                <a href={link.href} target="_blank" rel="noopener noreferrer" aria-label={`${link.label} 새 탭에서 열기`}>
            ...
            {teamMembers.map((member) => (
              <li key={member.name} title={member.details}>
                <div className="footer-member-heading">
                  ...
                  {member.github ? (
                    <a href={member.github} target="_blank" rel="noopener noreferrer" aria-label={`${member.name} GitHub 새 탭에서 열기`}>GitHub ↗</a>
