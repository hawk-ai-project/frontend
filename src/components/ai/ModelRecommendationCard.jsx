...
export default function ModelRecommendationCard({
  ...
  return <section className={styles.card} aria-live="polite">
    ...
    {unavailable?<p className={styles.state}>{unavailable}</p>:loading&&!recommendation?<p className={styles.state}>AI가 후보 모델을 비교하고 있습니다.</p>:error&&!recommendation?<p className={`${styles.state} ${styles.error}`}>{error}</p>:rankedRecommendations.length>0?<>
      ...
      <article className={`${styles.recommendation} ${styles.primary}`}><div className={styles.rankLine}><span className={styles.rank}>#{rankedRecommendations[0].rank ?? 1}</span><span className={styles.label}>{rankedRecommendations[0].label}</span>{currentModelId===rankedRecommendations[0].modelId&&<span className={styles.current}>현재 사용 중</span>}</div><h3 className={styles.modelName}>{rankedRecommendations[0].modelName}</h3><RecommendationDetails item={rankedRecommendations[0]}/>{action(rankedRecommendations[0])}</article>
      {rankedRecommendations.length>1&&<div className={styles.secondaryGrid}>{rankedRecommendations.slice(1).map((item,index)=><article className={styles.recommendation} key={`${item.modelId}-${index}`}><div className={styles.rankLine}><span className={styles.rank}>#{item.rank ?? index+2}</span><span className={styles.label}>{item.label}</span>{currentModelId===item.modelId&&<span className={styles.current}>현재 사용 중</span>}</div><h3 className={styles.modelName}>{item.modelName}</h3><RecommendationDetails item={item}/>{action(item)}</article>)}</div>}
