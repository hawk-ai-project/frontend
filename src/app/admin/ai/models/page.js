...
export default function AiModelsPage() {
  ...
  const loadRecommendation = useCallback(async () => {
    ...
    catch (requestError) { setRecommendationError(requestError.response?.status===404?"정기 AI 추천이 아직 생성되지 않았습니다. 잠시 후 다시 확인해 주세요.":requestError.response?.status===400?"선정 후보 모델이 없습니다. 관리자 AI 관리에서 비교할 모델을 후보로 등록해 주세요.":getApiErrorMessage(requestError,"AI 모델 추천을 불러오지 못했습니다.")); }
  ...
  return <div className="admin-page ai-models-page">
    ...
    <ModelRecommendationCard recommendation={recommendation} loading={recommendationLoading} error={recommendationError} stale={recommendationStale} description="후보 모델의 성능과 현재 GPU 상태를 종합한 정기 추천입니다." onSelectModel={(modelId)=>{const model=(models?.models||[]).find((item)=>item.id===modelId);if(model)void select(model);}} selectedModelId={models?.selectedModelId} selectableModels={models?.models||[]} selectingModelId={selecting}/>
