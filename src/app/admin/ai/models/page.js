...
export default function AiModelsPage() {
  ...
  return <div className="admin-page ai-models-page">
    ...
    <section className="admin-panel ai-experiment-panel"><div className="admin-toolbar"><div><h2>실험 추적</h2><p>실험을 클릭하면 상세 페이지에서 설정, 지표와 생성 이미지를 확인할 수 있습니다. {models ? `${models.models.length}개 실험 · ${models.runsDirectory}` : "AI/runs에서 학습 실험을 읽고 있습니다."}</p></div></div>{loading && !models ? <div className="admin-data-loading"><span className="admin-spinner" />실험 정보를 불러오고 있습니다.</div> : <div className="admin-table-wrap"><table className="admin-table ai-experiment-table"><thead><tr><th>실험</th><th>기반 모델</th><th>학습 설정</th><th>최종 지표</th><th>산출물</th><th>갱신</th><th>선택</th></tr></thead><tbody>{(models?.models || []).map((model) => <tr key={model.id} className={models.selectedModelId === model.id ? "ai-selected-row" : ""} onClick={() => router.push(`/admin/ai/models/${model.id}`)}><td><strong>{model.name}</strong><small>{model.path}</small></td><td>{model.model || "-"}</td><td>{model.optimizer || "-"}<small>{model.epochs ? `${model.epochs} epochs · ${model.imageSize || "-"}px · batch ${model.batch || "-"}` : "-"}</small></td><td><div className="ai-metrics"><span>mAP50 <b>{percent(model.metrics?.map50)}</b></span><span>mAP50-95 <b>{percent(model.metrics?.map50_95)}</b></span><span>Precision <b>{percent(model.metrics?.precision)}</b></span></div></td><td>{number(model.artifactCount)}개<small className={model.hasWeights ? "ai-weight-ready" : ""}>{model.hasWeights ? "best.pt 준비됨" : "가중치 없음"}</small></td><td>{date(model.updatedAt)}</td><td><button type="button" className="ai-select-btn" disabled={!model.hasWeights || selecting === model.id || models.selectedModelId === model.id} onClick={(event) => { event.stopPropagation(); void select(model); }}>{selecting === model.id ? "적용 중" : models.selectedModelId === model.id ? "사용 중" : model.hasWeights ? "이 모델 사용" : "선택 불가"}</button></td></tr>)}{models && !models.models.length && <tr><td colSpan="7" className="admin-empty-cell">AI/runs에 실험 결과가 없습니다.</td></tr>}</tbody></table></div>}</section>
  </div>;
}
