...
export default function AiModelsPage() {
  ...
  const loadRecommendation = useCallback(async () => {
    ...
    try { setRecommendation(await modelRecommendationService.recommendGlobal()); setRecommendationStale(false); }
    catch (requestError) { setRecommendationError(requestError.response?.status===400?"선정 후보 모델이 없습니다. 관리자 AI 관리에서 비교할 모델을 후보로 등록해 주세요.":getApiErrorMessage(requestError,"AI 모델 추천을 불러오지 못했습니다.")); }
  ...
  useEffect(() => { const timer=setTimeout(()=>void loadRecommendation(),0); return()=>clearTimeout(timer); }, [loadRecommendation]);

  const select = async (model) => {
    setSelecting(model.id); setError("");
    try { setModels(await adminService.selectAiModel(model.id)); setRecommendation((current)=>current?{...current,currentModelId:model.id}:current); }
  ...
  const toggleCandidate = async (model) => {
    ...
    try { setModels(await adminService.setAiModelCandidate(model.id, !model.isCandidate)); setRecommendationStale(true); }
  ...
  const recommendedModel = (models?.models || []).find((model)=>model.id===recommendation?.recommendedModelId);
  ...
  return <div className="admin-page ai-models-page">
    ...
    <ModelRecommendationCard recommendation={recommendation} loading={recommendationLoading} error={recommendationError} stale={recommendationStale} description="후보 모델의 성능과 현재 GPU 상태를 종합한 참고용 추천입니다." onRefresh={()=>void loadRecommendation()} onSelectModel={recommendedModel?()=>void select(recommendedModel):undefined} selectDisabled={!recommendedModel?.hasWeights||models?.selectedModelId===recommendedModel?.id} selecting={selecting===recommendedModel?.id}/>
