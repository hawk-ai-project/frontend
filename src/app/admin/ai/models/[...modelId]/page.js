...
export default function AiModelDetailPage() {
  ...
  return <div className="admin-page ai-model-detail-page">
    ...
    <section className="admin-panel ai-class-metrics"><div className="admin-toolbar"><div><h2>탐지 클래스별 정확도</h2><p>accuracy는 TP / (TP + FP + FN) 기준이며 모델 추천 데이터로 DB에 저장됩니다.</p></div><span>{detail.classMetrics?.length || 0}개 클래스</span></div><div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>클래스</th><th>Accuracy</th><th>Precision</th><th>Recall</th><th>mAP50</th><th>mAP50-95</th><th>표본</th><th>측정 기준</th></tr></thead><tbody>{(detail.classMetrics || []).map(metric=><tr key={`${metric.classIndex}-${metric.className}`}><td><strong>{metric.className}</strong></td><td>{percent(metric.accuracy)}</td><td>{percent(metric.precision)}</td><td>{percent(metric.recall)}</td><td>{percent(metric.map50)}</td><td>{percent(metric.map50_95)}</td><td>{number(metric.support)}</td><td>{metric.metricSource === "OPERATIONAL_REVIEW" ? "운영 검수" : "검증 데이터"}</td></tr>)}{!detail.classMetrics?.length&&<tr><td colSpan="8" className="admin-empty-cell">클래스별 평가 산출물이 없습니다. 다음 학습부터 자동 생성됩니다.</td></tr>}</tbody></table></div></section>
