...
export default function AiDataBrowser(){
  ...
  const [filters,setFilters]=useState({keyword:"",result:"",tagIds:[]}),[page,setPage]=useState(1),[pageSize,setPageSize]=useState(24);
  const [error,setError]=useState(""),[notice,setNotice]=useState(""),[working,setWorking]=useState(false);
  const [bulkTag,setBulkTag]=useState("");
  const load=useCallback(async()=>{try{setError("");const [items,allTags]=await Promise.all([adminService.getAiData({page,pageSize,...filters}),adminService.getAiTags()]);setData(items);setTags(allTags);}catch(e){setError(getApiErrorMessage(e));}},[filters,page,pageSize]);
  ...
  return <div className="admin-page ai-data-page">
    ...
    <section className="admin-panel data-filter-panel"><div className="admin-toolbar data-browser-toolbar"><div><h2>운영 데이터</h2><p>이미지, 탐지 클래스와 환경 태그를 조합해 조회합니다.</p></div><div className="admin-search-form">
      <input placeholder="점검명 또는 장소 검색" value={filters.keyword} onChange={e=>{setPage(1);setFilters(v=>({...v,keyword:e.target.value}));}}/>
      ...
      <select value={filters.tagIds[0]||""} onChange={e=>{setPage(1);setFilters(v=>({...v,tagIds:e.target.value?[Number(e.target.value)]:[]}));}}><option value="">전체 태그</option>{tags.map(tag=><option key={tag.id} value={tag.id}>[{tag.categoryName}] {tag.name} ({tag.usageCount})</option>)}</select><label className="data-page-size"><span>페이지당</span><select aria-label="페이지당 표시 개수" value={pageSize} onChange={e=>{setPage(1);setSelected([]);setPageSize(Number(e.target.value));}}><option value="12">12개</option><option value="24">24개</option><option value="48">48개</option><option value="96">96개</option></select></label>
    </div></div></section>
    ...
    {data&&<nav className="admin-pagination" aria-label="AI 데이터 페이지"><button type="button" disabled={page<=1} onClick={()=>setPage(p=>p-1)}>이전</button>{pageNumbers.map(pageNumber=><button type="button" key={pageNumber} className={pageNumber===page?"active":""} aria-current={pageNumber===page?"page":undefined} onClick={()=>setPage(pageNumber)}>{pageNumber}</button>)}<span>{page} / {totalPages} · 총 {data.totalItems.toLocaleString()}개</span><button type="button" disabled={page>=totalPages} onClick={()=>setPage(p=>p+1)}>다음</button></nav>}
  </div>;
}
